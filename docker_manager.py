...
def create_docker_container(server_id, server_name, server_path, jar_file, java_args='-Xmx1G -Xms1G'):
    ...
    cmd = json.dumps(['java'] + __import__('shlex').split(java_args) + ['-jar', jar_file, 'nogui'])
...
def stop_docker_container(container_name):
    if not check_docker(): return False, 'Docker not available'
    try:
        for cmd in (['docker', 'stop', container_name], ['docker', 'rm', container_name]):
            subprocess.run(cmd, capture_output=True, text=True, timeout=30)
        return True, 'Container stopped and removed'
    except: return False, 'Failed to stop Docker container'

def get_docker_logs(container_name, tail=100):
    if not check_docker(): return None, 'Docker not available'
    try:
        r = subprocess.run(['docker', 'logs', '--tail', str(tail), container_name], capture_output=True, text=True, timeout=10)
        return r.stdout + r.stderr, None
    except: return None, 'Failed to get Docker logs'

def docker_exec_command(container_name, command):
    if not check_docker(): return None, 'Docker not available'
    try:
        r = subprocess.run(['docker', 'exec', '-i', container_name] + command.split(), capture_output=True, text=True, timeout=30)
        return r.stdout + r.stderr, None
    except: return None, 'Docker exec failed'
